...
class SeqModel():
    ...
    def set_HKY(self,kappa=1, stationary_distribution=[0.25,0.25,0.25,0.25]):
    ...
    def _evolve_branch_probs(self,
                             br_len,
                             Q):
        return(expm(Q*br_len))
    
    # start with a sequence and probability matix, and make substitutions across the sequence
    def _substitute(self,
        ...
        for i in range(len(parent_seq)):
            # store a random choice of base in each index, based on probabilities associated with starting base
